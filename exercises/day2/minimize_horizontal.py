...
guids = compas_rhino.get_points("points")
points = compas_rhino.get_point_coordinates(guids)

guid = compas_rhino.get_polylines("boundary")[0]
boundary = compas_rhino.get_polyline_coordinates(guid)

guids = compas_rhino.get_polylines("holes")
...
# iterate to find smallest horizontal reaction
tot_reactions = []
zmaxes = []
for i in range(10):
    zmax = 5 + (.5*i)
    zmaxes.append(zmax)
    vertical_from_zmax(form, force, zmax=zmax, kmax=20)
    
    anchors = form.vertices_where({'is_anchor': True})
    reac = form.get_vertices_attributes(('rx', 'ry'), keys=anchors)
    reac = [length_vector_xy(r) for r in reac]
    tot_reactions.append(sum(reac))
    
    # draw the result
    
    artist = FormArtist(form, layer='FormDiagram{0}'.format(i))
    artist.clear_layer()
    ...
    artist.draw_reactions(scale=.01)
...
for i in range(len(tot_reactions)):
    print(i, tot_reactions[i], zmaxes[i])